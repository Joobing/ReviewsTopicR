...
tpcTMSH50 <- topics(TMSH50, threshold = 0.2)
tpcTMSH50
# 
...
dfTMSH50 <- data.frame(word = do.call(c, tpcTMSH50),
                           group = rep(1:length(tpcTMSH50), 
                                       sapply(tpcTMSH50, length)))

melt(tpcTMSH50)
###############write.table(dfTMSH50, file = file.path(Rdir, "tpcTMSH50-v1.xls"), sep='\t',col.names = FALSE)
write.csv(trmTMSH50, file.path(Rdir, "TMSH50.csv"))
TMSHvis<-topicmodels_json_ldavis(TMSH50, TMSH, clean(fromJSON(file.path(PYdir, "TMSH.json")) %>% as.data.frame))
