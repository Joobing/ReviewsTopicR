...
tpcTMLN50 <- topics(TMLN50, threshold = 0.2)
tpcTMLN50
# 
...
dfTMLN50 <- data.frame(word = do.call(c, tpcTMLN50),
                           group = rep(1:length(tpcTMLN50), 
                                       sapply(tpcTMLN50, length)))

melt(tpcTMLN50)
###############write.table(dfTMLN50, file = file.path(Rdir, "tpcTMLN50-v1.xls"), sep='\t',col.names = FALSE)
write.csv(trmTMLN50, file.path(Rdir, "TMLN50.csv"))
TMLNvis<-topicmodels_json_ldavis(TMLN50, TMLN, clean(fromJSON(file.path(PYdir, "TMLN.json")) %>% as.data.frame))
