...
tpcLGLN50 <- topics(LGLN50, threshold = 0.2)
tpcLGLN50
# 
...
dfLGLN50 <- data.frame(word = do.call(c, tpcLGLN50),
                           group = rep(1:length(tpcLGLN50), 
                                       sapply(tpcLGLN50, length)))

melt(tpcLGLN50)
###############write.table(dfLGLN50, file = file.path(Rdir, "tpcLGLN50-v1.xls"), sep='\t',col.names = FALSE)
write.csv(trmLGLN50, file.path(Rdir, "LGLN50.csv"))
LGLNvis<-topicmodels_json_ldavis(LGLN50, LGLN, clean(fromJSON(file.path(PYdir, "LGLN.json")) %>% as.data.frame))
