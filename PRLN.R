...
tpcPRLN50 <- topics(PRLN50, threshold = 0.2)
tpcPRLN50
# 
...
dfPRLN50 <- data.frame(word = do.call(c, tpcPRLN50),
                           group = rep(1:length(tpcPRLN50), 
                                       sapply(tpcPRLN50, length)))

melt(tpcPRLN50)
###############write.table(dfPRLN50, file = file.path(Rdir, "tpcPRLN50-v1.xls"), sep='\t',col.names = FALSE)
write.csv(trmPRLN50, file.path(Rdir, "PRLN50.csv"))
PRLNvis<-topicmodels_json_ldavis(PRLN50, PRLN, clean(fromJSON(file.path(PYdir, "PRLN.json")) %>% as.data.frame))
